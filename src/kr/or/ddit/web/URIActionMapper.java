...
public class URIActionMapper {
	...
	//static 초기화 블럭
	static {
		// properties 파일을 읽어와 Bundle객체를 생성한다.
		...
		while(en.hasMoreElements()) { // 키값 개수만큼 반복 처리
			...
			String value = rb.getString(key); // value값(실핼할 Action클래스명)
			
			actionMap.put(key, value); // ket값과 value값을 Map에 저장
		}
		
	}// static 초기화 블럭 끝..
	
	//매개값으로 주어진  uri에 맞는 Action객체를 인스턴스화해서 반환하는메서드 
	public static IAction getAction(String uri) {
		...
		if(actionMap.containsKey(uri)) {
			try {
				...
				// 메모리에 로딩된 Action클래스를 '인스턴스'화한다. ==> 즉. 객체를 생성한다.
